...
export JSON=release.json
export DATASET_ID=az_devops_cd
export TABLE_ID=table-azure-cd
export CSV_FILE=$CSV
export SCHEMA_FILE=release.json
export PROJECT_ID=""
...
for i in `head -1 $CSV | tr ',' '\n' | sed 's/^ //' | tr -d '"'| tr ' ' '_'`; do
  ...
  elif echo $i | grep -i Duracao 2> /dev/null ; then
